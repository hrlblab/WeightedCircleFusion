    ...
    def __init__(self, opt, split):
        ...
        if split == 'test':
            self.annot_path = os.path.join(
                self.data_dir, 'mou_glo_1024_test_circle.json').format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                    self.data_dir, 'auto_label_circle.json').format(split)
            else:
            ##change path to mou_glo,change format(split)
                self.annot_path = os.path.join(
                    self.data_dir, 'mou_glo_1024_{}_circle.json'.format(split))
