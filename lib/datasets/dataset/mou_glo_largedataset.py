...
class MouGloL(data.Dataset):
    ...
    def __init__(self, opt, split):
        super(MouGloL, self).__init__()
        #self.data_dir = os.path.join(opt.data_dir, 'mou_glo')
        self.data_dir=os.path.join(opt.data_dir,'five_fold_json')
        #self.img_dir = os.path.join(self.data_dir, '{}'.format(split))
        self.img_dir = os.path.join(self.data_dir)
        if split == 'test':
            test_dir=os.path.join(self.data_dir,'test')
            self.annot_path = os.path.join(
                test_dir, 'fold5_val_circle.json').format(split)
        else:
            if opt.task == 'exdet':
                self.annot_path = os.path.join(
                    self.data_dir, 'database123.json').format(split)
            elif split == 'val':
                val_dir = os.path.join(self.data_dir, 'val')
                self.annot_path = os.path.join(
                    val_dir, 'fold5_val_circle.json').format(split)
            else:
            ##change path to mou_glo,change format(split)
                train_dir = os.path.join(self.data_dir, 'train')
                #train_dir=os.path.join(train_dir,'all_train_data')
                self.annot_path = os.path.join(
                    train_dir, 'SPIE_model5_withczi.json'.format(split))

        #self.max_objs = 128
        self.max_objs = 1000
        self.class_name = [
           '__background__', 'glomerulus']
        #self.class_name = ['glomerulus']
        ##change
        self._valid_ids = [1]
        ...
        print('==> initializing mouse glomerular 2023 {} data.'.format(split))
    ...
    def run_eval(self, results, save_dir):
        ...
        #add
        #iou_thresholds = np.linspace(.0, 0.95, int(np.round((0.95 - .0) / .05)) + 1, endpoint=True)
