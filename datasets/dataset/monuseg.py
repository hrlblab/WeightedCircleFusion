...
class MoNuSeg(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        self.data_dir = os.path.join(opt.data_dir, 'monuseg')
        data_dir='/data/CircleNet/data/new_data_for_miccai_paper/test'
        self.img_dir = os.path.join(data_dir, '{}'.format(split))
        if opt.task == 'exdet':
            self.annot_path = os.path.join(
                data_dir, 'MoNuSeg_extreme_{}2021.json').format(split)
        else:
            self.annot_path = os.path.join(
                data_dir, '{}_updated.json').format(split)
        self.max_objs = 1000
        self.class_name = [
        ...
        print('==> initializing MoNuSeg 2021 {} data.'.format(split))
