...
def soft_nms_linear_decay(overlap_ratio, original_score, decay=0.5):
    """
    Apply a linear decay to the score based on overlap.
    decay: The factor by which the score is reduced for each unit of overlap.
    """
    #overlap_ratio = overlap_area / total_area
    return original_score * (1 - decay * overlap_ratio)
...
def process_xml_files(xml_files):
    ...
    """
    existing_regions = []
    overlap_found=False# Initialize with regions from the first file
        ...
        if file_index > 0:  # For subsequent files, apply the filtering logic
            ...
            for new_region in new_regions:
                ...
                for i, existing_region in enumerate(existing_regions):
                    ...
                    if IOU > 0.5:
                        # If the overlap is very high, remove the one with the lower 'text' value
                        if float(new_region['text']) > float(existing_region['text']):
                            # Replace existing region with new region
                            existing_regions[i] = new_region
                            overlap_found=True
                        # If the existing region has a higher 'text', or they are equal, do nothing (new_region is discarded)
                        break
                    elif IOU > 0.3:
                        # Apply decay to the 'text' score of the existing region for moderate overlaps
                        decayed_score_exist = soft_nms_linear_decay(IOU, float(existing_region['text']))
                        decayed_score_new = soft_nms_linear_decay(IOU, float(new_region['text']))

                        existing_region['text'] = str(round(decayed_score_exist, 2))
                        new_region['text']=str(round(decayed_score_new, 2))
    ...
    output_file = os.path.join(folder_path, folder_name + "_softnms_iou_new.xml")
...
#root_folder = '/data/CircleNet/data/WSI_raw_data/14_hy_xml/test_if_model_change/merged_xml'
