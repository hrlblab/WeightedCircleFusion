...
def scn_to_patchs(scn_file, working_dir, opt):
    ...
    for xi in range(num_overlap_patch_x):
        for yi in range(num_overlap_patch_y):
            ...
            rotated_img = cv2.rotate(img_patch, cv2.ROTATE_90_CLOCKWISE)
            ...
            # Save the flipped patch
            cv2.imwrite(patch_file, rotated_img)
...
def run_one_scn(demo_scn, working_dir,detector,basename,opt,xml_file):
    ...
    for (image_name) in image_names:
        ...
        image_width = 512  # Image width before rotation
        image_height = 512  # Image height before rotation (assuming a square image for simplicity)

        for key in results.keys():
            bboxes = results[key]  # Assuming bboxes structure: [centerx, centery, widthx, heighty, score]
            for bbox in bboxes:
                old_centerx, old_centery= bbox[:2]

                # Adjust coordinates and dimensions for 90-degree clockwise rotation
                new_centerx = old_centery
                new_centery = image_width - old_centerx  # Assuming the image is square
                bbox[0]=new_centerx
                bbox[1]=new_centery
