...
def main(opt):
    ...
    if opt.test:
        ...
        if not opt.rotate_reproduce:
            if opt.task == 'circledet':
                ...
                #add mean raidus diff and mean center distance calculation
                # true_results_json_path = '../data/mou_glo/mou_glo_1024_test_circle.json'
                #
                # mean_radius_diff, mean_distance_diff = calculate_radius_and_distance(
                #     preds, true_results_json_path)
                #
                # # Print or use the calculated differences and mean values
                # print("Mean Radius Difference:", mean_radius_diff)
                # print("Mean Center Point Distance:", mean_distance_diff)
            ...
            if opt.task == 'circledet':
                ...
                preds2_rot = correct_rotate_circle(preds2_rot, 256, 256, 90)
                ...
                preds2_rot = correct_rotate(preds2_rot, 256, 256, 90)
    ...
    best = 1e10
    #add save_dir. orginal:opt.save_dir
    save_dir=os.path.join(opt.save_dir,'SPIE_model5_withczi')
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            save_model(os.path.join(save_dir, 'model_{}.pth'.format(mark)),
            ...
            if log_dict_val[opt.metric] < best:
                ...
                save_model(os.path.join(save_dir, 'model_best.pth'),
                           epoch, model)
        else:
            save_model(os.path.join(save_dir, 'model_last.pth'),
                       epoch, model, optimizer)
        logger.write('\n')
        if epoch in opt.lr_step:
            save_model(os.path.join(save_dir, 'model_{}.pth'.format(epoch)),
                       epoch, model, optimizer)
...
#add calculate_radius_and_distance for one glo detection
# def calculate_radius_and_distance(test_results, true_results_json_path):
#     # Load true results from the JSON file
#     with open(true_results_json_path, 'r') as file:
#         true_results = json.load(file)
#
...
#     # Iterate over each test result
#     for index, sub_dict in test_results.items():
#         for key, test_vectors in sub_dict.items():
#             # Extract relevant information from the test result vectors
#             test_vector = test_vectors[0]  # Assuming there's only one vector in the list
#             test_radius = test_vector[2]  # assuming radius is at index 2
#             test_center = (test_vector[0], test_vector[1])  # center_x and center_y
#
#             # Extract the corresponding true result (assuming a matching key exists)
#             true_vector = true_results.get('annotations', None)
#
#             if true_vector:
#                 for item in true_vector:
#                     true_center = item.get('circle_center', [0, 0])
#                     true_radius = item.get('circle_radius', 0)
#
#                     # Calculate the difference in radius
#                     radius_difference = abs(test_radius - true_radius)
#                     radius_differences.append(radius_difference)
#
#                     # Calculate the distance between center points
#                     distance = math.sqrt((test_center[0] - true_center[0]) ** 2 + (test_center[1] - true_center[1]) ** 2)
#                     center_point_distances.append(distance)
#
#     # Calculate mean values
#     mean_radius_difference = sum(radius_differences) / len(radius_differences) if radius_differences else 0
#     mean_center_point_distance = sum(center_point_distances) / len(center_point_distances) if center_point_distances else 0
#
#     return  mean_radius_difference, mean_center_point_distance
